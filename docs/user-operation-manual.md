...
| 您的帳號不在允許的登入群組中 | AD 群組不符合系統允許清單 | 聯絡管理者確認 AD 群組。 |
| 您的帳號未獲得授權 | 系統使用者清單尚未建立該帳號 | 聯絡 Admin 加入使用者。 |
| 您的帳號已被停用 | 管理者停用帳號 | 聯絡 Admin 確認帳號狀態。 |
| 登入失敗。請聯絡管理員 | LDAP、資料庫或系統例外 | 聯絡管理者查看系統錯誤紀錄。 |
