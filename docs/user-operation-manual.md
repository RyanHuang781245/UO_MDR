...
主要使用者包含一般編輯者與系統管理者。編輯者可建立與操作文件轉換任務、流程、Mapping、標準更新任務與批次排程；系統管理者另可進入系統後台管理使用者、查詢帳號、調整系統設定、檢視操作紀錄與系統錯誤紀錄。
...
| 欄位名稱  | 說明 | 格式或限制 |
| ---- | -- | ----- |
| 任務 ID | 任務唯一識別碼 | 8 碼 UUID 前綴，依程式碼產生。 |
| 建立人員 | 建立任務的使用者 | 來自登入者資訊。 |
| NAS 存取路徑 | 任務來源資料夾 | 需可讀取。 |
| 同步狀態 | queued、running、failed 或完成 | 顯示為排隊中、同步中、失敗、完成。 |
...
6. 點選「保存流程」保存定義，或點選「執行流程」送出背景執行流程。
   ![alt text](image-32.png)
7. 點選保存流程會存放至「已保存流程」;執行流程後於「流程執行結果」或「輸出存放空間」查看結果。
   已保存流程
